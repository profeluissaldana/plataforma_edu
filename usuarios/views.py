...
def tomar_asistencia(request):
    ...
    cursos = Usuario.objects.filter(is_superuser=False, is_staff=False)\
    ...
    if curso_filtro:
        ...
        alumnos_qs = Usuario.objects.filter(
            is_superuser=False, 
            is_staff=False, 
            curso__iexact=curso_filtro.strip()
        )
    ...
    if request.method == 'POST':
        ...
        alumnos_post = Usuario.objects.filter(
            is_superuser=False, 
            is_staff=False, 
            curso__iexact=curso_post.strip()
        )
        ...
        messages.success(request, f"Asistencias guardadas correctamente para {curso_post} el {fecha_post}.")
...
def descargar_plantilla_excel(request):
    ...
    data = {
        ...
        'email': ['alumno01@escuela.edu.ar', 'alumno02@escuela.edu.ar', 'alumno03@escuela.edu.ar'],
...
def lista_alumnos(request):
    ...
    cursos = Usuario.objects.filter(is_superuser=False, is_staff=False).values_list('curso', flat=True).distinct()
    dias = Usuario.objects.filter(is_superuser=False, is_staff=False).values_list('dia_cursado', flat=True).distinct()
...
def editar_alumno(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f"Se actualizaron los datos de {alumno.get_full_name() or alumno.username}.")
            return redirect('lista_alumnos')
    else:
        form = EditarUsuarioForm(instance=alumno)
    return render(request, 'usuarios/editar_alumno.html', {'form': form, 'alumno': alumno})
...
def reset_password_alumno(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            alumno.save()
            messages.success(request, f"Contraseña actualizada con éxito para el usuario '{alumno.username}'.")
            return redirect('lista_alumnos')
    else:
        form = ResetPasswordForm()
    return render(request, 'usuarios/reset_password.html', {'form': form, 'alumno': alumno})
...
def eliminar_alumno(request, pk):
    ...
    if request.method == 'POST':
        ...
        alumno.delete()
        messages.success(request, f"El alumno {nombre} fue eliminado correctamente.")
        return redirect('lista_alumnos')
...
def exportar_alumnos_pdf(request):
    ...
    subtitulo += ", ".join(detalles_filtros) if detalles_filtros else "Ninguno (Todos los alumnos)"
    elements.append(Paragraph(subtitulo, styles['Normal']))
    elements.append(Spacer(1, 15))

    table_data = [['Usuario', 'Nombre Completo', 'DNI', 'Curso', 'Turno', 'Día Cursado']]
